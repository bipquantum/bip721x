...
const BipItem: React.FC<BipItemProps> = ({
  ...
  const { data: author } = backendActor.unauthenticated.useQueryCall({
    ...
    args: [intProp.V1.author ?? Principal.anonymous()],
  });
  ...
  useEffect(() => {
    setCanDelete(
      principal !== undefined &&
        intProp.V1.author !== undefined &&
        intProp.V1.author.compareTo(principal) === "eq" &&
        owner !== undefined &&
        owner?.compareTo(principal) === "eq",
    );
  }, [principal, intProp.V1.author, owner]);
        ...
        <div className="flex h-full flex-col justify-between pt-3">
          <div className="flex flex-col items-start justify-between gap-2 md:flex-row md:gap-0">
            <div className={`flex w-full flex-row gap-2`}>
              <UserImage principal={intProp.V1.author} />
                ...
                <div className="w-fit pt-1">
                  <p className="text-sm font-light text-neutral-400">
                    ...
                    <span className="text-sm font-light text-neutral-400">
                      {intPropTypeToString(intProp.V1.intPropType)}
                    </span>
                  </p>
                  <p className="text-sm font-light text-neutral-400">
                    Licenses:{" "}
                    <span className="text-sm font-light text-neutral-400">
                      {intProp.V1.intPropLicenses &&
                      intProp.V1.intPropLicenses.length > 0
                        ? intProp.V1.intPropLicenses
