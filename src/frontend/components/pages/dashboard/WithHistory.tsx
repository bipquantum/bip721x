...
const getCustomStateInfo = (stateValue: any): CustomStateInfo => {
  ...
  const statePath =
    typeof stateValue === "string" ? [stateValue] : Object.keys(stateValue);
...
const WithHistory: React.FC<WithHistoryProps> = ({ principal, chatId }) => {
  ...
  actor.subscribe((state) => {
    if (state.value !== undefined) {
      ...
      if (
        ...
        chats.current[chats.current.length - 1].key !== key
      ) {
        const metaDescription =
          state.getMeta()["chat.bipCertificate"]?.description;
        const description =
  ...
  const [aiPrompts, setAIPrompts] = useState<Map<number, AiPrompt[]>>(
    new Map(),
  );
  ...
  const refreshChat = () => {
    ...
    if (
      actor.getSnapshot().value !== machine.definition.initial?.target[0].key
    ) {
      chats.current = [];
      send({ type: "reset" });
    }
    ...
    updateChatHistory([
      {
        ...
        aiPrompts: JSON.stringify(Array.from(aiPrompts.entries())),
      },
    ]).catch((error) => {
